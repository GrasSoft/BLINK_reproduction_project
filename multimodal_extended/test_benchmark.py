...
def analyze_answer(d, model_answer: str, all_choices: list[str], model_name: str):
    """
    Extracts the multiple choice answer from a long paragraph of model output if there is only one choice.
    Otherwise, query the model to extract the choice.
    If the model output is short and only contains the choice, reformats the choice in the correct format;
    e.g. (A) and returns the choice as is.
    ...
    try:
        ...
        intersect_last = list(
            set(all_choices).intersection(set(model_answer.split("\n\n")[-1].split()))
        )
...
def query_model(task_name: str, model_name: str):
    """
...
def eval_task(task_name, model_name):
    outputs = query_model(task_name, model_name)
    accu = {"val": 0, "test": 0}
    for split in ["val", "test"]:
...
def parse_args():
    ...
    parser.add_argument(
        "--task_name", type=str, default="Art Style", help="select the task name"
    )
...
if __name__ == "__main__":
    ...
    model_generate_func = lambda image_paths, prompt: query_ollama(
        image_paths, prompt, model_name
    )
    ...
    for task_name in subtasks:
        eval_task(task_name, model_name)
